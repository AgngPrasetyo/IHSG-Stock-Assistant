(() => {
    ...
    try {
      ...
      select.innerHTML =
        ...
        body.data.map((stock) => {
          ...
          const name = stock.stock_name ? ` — ${escapeHtml(stock.stock_name)}` : '';
          const sector = stock.sektor ? ` — ${escapeHtml(stock.sektor)}` : '';
          return `<option value="${ticker}">${ticker}${name}${sector}</option>`;
        }).join('');
    } catch {
  ...
  function setState(name) {
    // Animate state changes while preserving hidden attributes for accessibility.
    if (!stateMap[name]) return;

    clearTimeout(stateTransitionTimer);

    const previousName = currentState;
    const previous = stateMap[previousName];
    ...
    if (prefersReducedMotion() || !previous || previous === next || previous.hidden) {
      activateNext();
      return;
    }
  ...
  function render(body) {
    ...
    $('#result-numerical').innerHTML = `
      <div class="result-stack">
        ...
        <section class="result-card">
          <h3 class="metrics-title">Perbandingan indikator sektor</h3>
          <div class="chart-wrap">
