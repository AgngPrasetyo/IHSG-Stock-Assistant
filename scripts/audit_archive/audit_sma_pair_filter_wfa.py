...
WINDOW_OUTPUT_PATH = OUTPUT_DIR / "audit_sma_pair_filter_wfa_window_results.csv"
SUMMARY_OUTPUT_PATH = OUTPUT_DIR / "audit_sma_pair_filter_wfa_summary.csv"
SECTOR_OUTPUT_PATH = OUTPUT_DIR / "audit_sma_pair_filter_wfa_sector_aggregate.csv"
BEST_OVERALL_OUTPUT_PATH = OUTPUT_DIR / "audit_sma_pair_filter_wfa_best_overall.csv"
BEST_BY_SECTOR_OUTPUT_PATH = OUTPUT_DIR / "audit_sma_pair_filter_wfa_best_by_sector.csv"
ERROR_OUTPUT_PATH = OUTPUT_DIR / "audit_sma_pair_filter_wfa_errors.csv"
...
MA_PAIR_CONFIGS = [
    {"pair_label": "SMA10/SMA50", "fast_window": 10, "slow_window": 50},
    {"pair_label": "SMA20/SMA50", "fast_window": 20, "slow_window": 50},
    {"pair_label": "SMA50/SMA200", "fast_window": 50, "slow_window": 200},
]
...
def build_variant_configs() -> list[dict[str, Any]]:
    ...
    for pair in MA_PAIR_CONFIGS:
        for threshold in THRESHOLD_CONFIGS:
            for volume in VOLUME_CONFIGS:
                fast_window = int(pair["fast_window"])
                slow_window = int(pair["slow_window"])
                ...
                signal_column = (
                    f"SMA_{fast_window}_{slow_window}_"
                    f"TH_{threshold_code}_{volume_code}_Signal"
                )
...
def generate_sma_crossover_signal(
    ...
    result = ensure_sma_columns(df, fast_window, slow_window)
...
def run_stock_wfa(stock: pd.Series) -> list[dict[str, Any]]:
    ...
    for window in windows:
        ...
        for variant in VARIANT_CONFIGS:
            signal_df = generate_sma_crossover_signal(
                signal_df,
...
def main() -> None:
    ...
    print(f"Jumlah varian SMA Crossover yang diuji: {len(VARIANT_CONFIGS)}")
    print("Mulai menjalankan audit SMA pair + filter...\n")
    ...
    print("\nPerbandingan terbaik per pasangan SMA:")
    best_by_pair_df = (
        best_overall_df.sort_values(
            ["pair_label", "directional_accuracy", "hit_rate", "total_active_signals"],
            ascending=[True, False, False, False],
        )
