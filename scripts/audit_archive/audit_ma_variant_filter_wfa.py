...
WINDOW_OUTPUT_PATH = OUTPUT_DIR / "audit_ma_variant_filter_wfa_window_results.csv"
SUMMARY_OUTPUT_PATH = OUTPUT_DIR / "audit_ma_variant_filter_wfa_summary.csv"
SECTOR_OUTPUT_PATH = OUTPUT_DIR / "audit_ma_variant_filter_wfa_sector_aggregate.csv"
BEST_OVERALL_OUTPUT_PATH = OUTPUT_DIR / "audit_ma_variant_filter_wfa_best_overall.csv"
BEST_BY_SECTOR_OUTPUT_PATH = OUTPUT_DIR / "audit_ma_variant_filter_wfa_best_by_sector.csv"
ERROR_OUTPUT_PATH = OUTPUT_DIR / "audit_ma_variant_filter_wfa_errors.csv"
...
MA_PAIRS = [
    {"ma_pair": "SMA20/SMA50", "fast_window": 20, "slow_window": 50},
    {"ma_pair": "SMA14/SMA21", "fast_window": 14, "slow_window": 21},
    {"ma_pair": "SMA10/SMA50", "fast_window": 10, "slow_window": 50},
]

THRESHOLD_CONFIGS = [
    {"threshold_label": "No Threshold", "threshold_pct": 0.0},
    {"threshold_label": "Threshold 0.10%", "threshold_pct": 0.001},
    {"threshold_label": "Threshold 0.25%", "threshold_pct": 0.0025},
    {"threshold_label": "Threshold 0.50%", "threshold_pct": 0.005},
]

VOLUME_CONFIGS = [
    {"volume_label": "No Volume Filter", "use_volume_filter": False, "volume_window": 20, "volume_multiplier": 0.0},
    {"volume_label": "Volume >= VolMA20", "use_volume_filter": True, "volume_window": 20, "volume_multiplier": 1.0},
]
    ...
    for ma in MA_PAIRS:
        for threshold in THRESHOLD_CONFIGS:
            for volume in VOLUME_CONFIGS:
                fast = int(ma["fast_window"])
                slow = int(ma["slow_window"])
                ...
                variant_name = (
                    f'{ma["ma_pair"]} | {threshold["threshold_label"]} | {volume["volume_label"]}'
                )

                signal_column = f"MA_{fast}_{slow}_TH_{threshold_code}_{volume_code}_Signal"
...
def generate_filtered_ma_signal(
    df: pd.DataFrame,
    fast_window: int,
    slow_window: int,
    signal_column: str,
    ...
    if use_volume_filter:
        if "Volume" not in result.columns:
            raise ValueError("Kolom Volume tidak tersedia untuk volume filter.")
...
def run_stock_wfa(stock: pd.Series) -> list[dict[str, Any]]:
    ...
    for window in windows:
        ...
        for variant in VARIANT_CONFIGS:
            signal_df = generate_filtered_ma_signal(
                signal_df,
            ...
            records.append(
                {
                    ...
                    "ma_pair": variant["ma_pair"],
                    "fast_window": int(variant["fast_window"]),
                    "slow_window": int(variant["slow_window"]),
                    "threshold_label": variant["threshold_label"],
                    "threshold_pct": float(variant["threshold_pct"]),
...
def main() -> None:
    ...
    print(f"Jumlah varian yang diuji per window: {len(VARIANT_CONFIGS)}")
    print("Mulai menjalankan WFA filter MA...\n")
    ...
    print("\nTop 10 varian keseluruhan yang memenuhi batas minimum active signal:")
    print(
        best_overall_df[
            [
                "ma_pair",
