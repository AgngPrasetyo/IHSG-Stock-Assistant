...
def generate_ma_final_signal(df: pd.DataFrame) -> pd.DataFrame:
    result = ensure_base_indicators(df)
    signal_column = "MA_Final_Signal"
    result[signal_column] = HOLD

    valid = (
        result[["SMA10", "SMA50", "Volume", "Volume_MA20"]].notna().all(axis=1)
    ...
    volume_confirmed = result["Volume"] >= result["Volume_MA20"] * 0.8

    result.loc[bullish_cross & volume_confirmed, signal_column] = BUY
    result.loc[bearish_cross & volume_confirmed, signal_column] = SELL

    return result


def generate_macd_final_signal(df: pd.DataFrame) -> pd.DataFrame:
    result = ensure_base_indicators(df)
    signal_column = "MACD_Final_Signal"
    result[signal_column] = HOLD
    ...
    bullish_signal = bullish_cross & (bullish_distance >= 0.001) & volume_confirmed
    bearish_signal = bearish_cross & (bearish_distance >= 0.001) & volume_confirmed

    result.loc[bullish_signal, signal_column] = BUY
    result.loc[bearish_signal, signal_column] = SELL

    return result


def generate_rsi_final_signal(df: pd.DataFrame) -> pd.DataFrame:
    result = ensure_base_indicators(df)
    signal_column = "RSI_Final_Signal"
    result[signal_column] = HOLD
...
def run_stock_wfa(stock: pd.Series) -> list[dict[str, Any]]:
    ...
    for window in windows:
        ...
        signal_df = generate_ma_final_signal(signal_df)
        signal_df = generate_macd_final_signal(signal_df)
        signal_df = generate_rsi_final_signal(signal_df)
...
def main() -> None:
    ...
    print("Mulai menjalankan audit final indikator...\n")
    ...
    print("\nPerbandingan indikator final per sektor:")
    print(
        sector_df[
            [
                "sector",
    ...
    print("\nIndikator terbaik final per sektor:")
    print(
        best_by_sector_df[
            [
                "sector",
