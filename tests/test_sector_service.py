...
def test_aggregate_sector_results_uses_all_active_window_hit_rates_not_stock_average():
    ...
    assert row["total_active_signals"] == 11
    assert row["correct_signals"] == 2
...
def test_aggregate_sector_results_zero_hit_rate_when_no_active_windows():
    window_rows = make_sector_window_results()
    window_rows["total_active_signals"] = 0
    window_rows["correct_signals"] = 0
    window_rows["hit_rate"] = 0.0

    result = aggregate_sector_results(window_rows)
    row = result[result["indicator"] == "MA Crossover"].iloc[0]
