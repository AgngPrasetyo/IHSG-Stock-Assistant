...
def test_build_llm_context_excludes_raw_internal_objects_and_chart_data(bbca_analysis):
...
def test_build_llm_prompt_contains_final_guardrails(bbca_analysis):
    prompt = build_llm_prompt(build_llm_context(bbca_analysis))
    prompt_lower = prompt.lower()

    assert "jangan mengubah sinyal" in prompt_lower
    assert "jangan menghitung ulang indikator" in prompt_lower
    assert "jangan memberi rekomendasi investasi final" in prompt_lower
    assert "average forward return" in prompt_lower
    assert "t+1, t+3, t+5, dan t+10" in prompt_lower
    assert "crossover sma10 dan sma50" in prompt_lower
    assert "crossover sma20 dan sma50" not in prompt_lower
    assert "horizon evaluasi 3 trading days" not in prompt_lower
    assert "jangan menyebut volma20" in prompt_lower
    assert "teks biasa tanpa markdown bold" in prompt_lower


def test_prompt_contains_correct_hold_guidance(bbca_analysis):
    prompt = build_llm_prompt(build_llm_context(bbca_analysis))
...
def test_build_llm_prompt_size_is_reasonable_for_guardrails(bbca_analysis):
    prompt = build_llm_prompt(build_llm_context(bbca_analysis))
...
def test_deterministic_explanation_valid_bbca(bbca_analysis):
    ...
    assert bbca_analysis["best_indicator"] in explanation
    assert bbca_analysis["latest_signal"] in explanation
    assert "Average Forward Return" in explanation
    assert "T+1, T+3, T+5, dan T+10" in explanation
...
def test_invalid_analysis_does_not_call_openai(monkeypatch):
    ...
    result = generate_llm_explanation({"success": False, "message": "Kode saham belum tersedia dalam mapping sistem."})
