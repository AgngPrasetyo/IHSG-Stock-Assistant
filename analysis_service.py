...
def build_latest_condition(df: pd.DataFrame, indicator_name: str) -> str:
    ...
    if name == "MA Crossover":
        ...
        return f"Close {close}; SMA20 {sma20}; SMA50 {sma50}. {position}."
    if name == "MACD":
        ...
        return f"MACD {macd}; MACD Signal {macd_signal}; Close {close}; SMA50 {sma50}. {trend}."

    rsi = _format_number(row.get("RSI"))
    rsi_state = _rsi_state(row.get("RSI"))
    trend = _relative_position(row.get("Close"), row.get("SMA50"), "Harga", "SMA50")
    return f"RSI {rsi} ({rsi_state}); Close {close}; SMA50 {sma50}. {trend}."
