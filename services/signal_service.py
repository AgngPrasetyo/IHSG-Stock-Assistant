...
def generate_ma_signal(df: pd.DataFrame) -> pd.DataFrame:
    """Signal MA Crossover SMA20/SMA50 entries and exits."""
    signal_df = _ensure_sma_columns(_prepare_signal_dataframe(df), 20, 50)
    signal_df[MA_CROSSOVER_SIGNAL_COLUMN] = HOLD
    valid = _has_current_and_previous_values(signal_df, ["SMA20", "SMA50"])
    buy = valid & (signal_df["SMA20"].shift(1) <= signal_df["SMA50"].shift(1)) & (signal_df["SMA20"] > signal_df["SMA50"])
    sell = valid & (signal_df["SMA20"].shift(1) >= signal_df["SMA50"].shift(1)) & (signal_df["SMA20"] < signal_df["SMA50"])
    signal_df.loc[buy, MA_CROSSOVER_SIGNAL_COLUMN] = BUY
    signal_df.loc[sell, MA_CROSSOVER_SIGNAL_COLUMN] = SELL
    return signal_df


def generate_macd_signal(df: pd.DataFrame) -> pd.DataFrame:
    """Signal MACD crosses confirmed by the SMA50 trend filter."""
    ...
    valid = _has_current_and_previous_values(signal_df, ["Close", "SMA50", "MACD", "MACD_Signal"])
    buy = valid & (signal_df["MACD"].shift(1) <= signal_df["MACD_Signal"].shift(1)) & (signal_df["MACD"] > signal_df["MACD_Signal"]) & (signal_df["Close"] > signal_df["SMA50"])
    sell = valid & (signal_df["MACD"].shift(1) >= signal_df["MACD_Signal"].shift(1)) & (signal_df["MACD"] < signal_df["MACD_Signal"]) & (signal_df["Close"] < signal_df["SMA50"])
...
def generate_rsi_signal(df: pd.DataFrame) -> pd.DataFrame:
    """Signal RSI exits from extremes confirmed by the SMA50 trend filter."""
    signal_df = _ensure_sma_columns(_prepare_signal_dataframe(df), 50)
...
def generate_all_signals(df: pd.DataFrame) -> pd.DataFrame:
    """Generate MA Crossover, MACD, and RSI final signal columns."""
    signal_df = _ensure_sma_columns(_prepare_signal_dataframe(df), 20, 50)
    signal_df = calculate_macd(signal_df)
    signal_df = calculate_rsi(signal_df, 14)
    return generate_rsi_signal(generate_macd_signal(generate_ma_signal(signal_df)))
...
def _normalize_indicator_name(indicator_name: str) -> str:
    ...
    if name in {"ma crossover", "moving average crossover", "sma crossover", "sma20/sma50"}: return "MA Crossover"
...
def _build_reason(row: pd.Series, indicator_name: str) -> str:
    name = _normalize_indicator_name(indicator_name)
    required = {"MA Crossover": ["Close", "SMA20", "SMA50"], "MACD": ["Close", "SMA50", "MACD", "MACD_Signal"], "RSI": ["Close", "SMA50", "RSI"]}[name]
    if any(pd.isna(row.get(column)) for column in required):
        return f"Nilai {name} atau SMA50 belum cukup, sehingga sinyal analisis teknikal HOLD."
    descriptions = {"MA Crossover": "MA Crossover SMA20/SMA50", "MACD": "MACD crossover dengan filter tren SMA50", "RSI": "RSI exit dari area ekstrem dengan filter tren SMA50"}
